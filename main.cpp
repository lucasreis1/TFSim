...
class clock_: public sc_module
{
	public:
		...
		clock_(sc_module_name name, int dl, nana::label &clk): sc_module(name), delay(dl), clock_count(clk)
		{
			SC_THREAD(main);
		}
		void main()
...
class res_station: public sc_module
{
	public:
		...
		res_station(sc_module_name name,int i, string n, map<string,int> inst_map, const nana::listbox::item_proxy item, const nana::listbox::cat_proxy c):sc_module(name), id(i), type_name(n), instruct_time(inst_map), table_item(item), cat(c)
		{
			Busy = isFirst = false;
			vj = vk = qj = qk = a = 0;
			SC_THREAD(exec);
...
class res_vector: public sc_module
{
	public:
		...
		res_vector(sc_module_name name,unsigned int t1, unsigned int t2,map<string,int> instruct_time, nana::listbox &lsbox, nana::listbox::cat_proxy ct):sc_module(name), table(lsbox)
		{
...
class sl_buffer: public sc_module
{
	public:
		...
		sl_buffer(sc_module_name name,unsigned int t,unsigned int t_outros,map<string,int> instruct_time, nana::listbox &lsbox, nana::listbox::cat_proxy ct): sc_module(name), tam(t), tam_outros(t_outros), table(lsbox)
		{
			string texto;
...
class top: public sc_module
{
		...
		vector<string> instruct_queue, nana::listbox &table, nana::grid &mem_gui, nana::listbox &regs, nana::listbox &instr, nana::label &ccount): sc_module(name)
	{
		...
		clk = new clock_("clock",1,ccount);
		fila = new instruction_queue("fila_inst",instruct_queue,instr);
		rst = new res_vector("rs_vc",t1,t2,instruct_time,table,instr.at(0));
		rb = new register_bank("register_bank", regs);
		slb = new sl_buffer("sl_buffer",t3,t1+t2,instruct_time,table,instr.at(0));
